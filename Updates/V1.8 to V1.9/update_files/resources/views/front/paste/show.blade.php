            ...
            <div class="card">
              <div class="card-header"> <span class="badge badge-light">{{strtoupper($paste->syntax)}}</span> <small class="text-muted">{{$paste->content_size}} KB</small>
                <div class="pull-right"> @if(config('settings.feature_share') == 1)<a class="badge badge-warning" data-toggle="modal" data-target="#shareModal">{{ __('share')}}</a> @endif
                  ...
                  @if(config('settings.feature_print') == 1 && empty($paste->password))<a href="{{url('print/'.$paste->slug)}}" class="badge badge-info">print</a> @endif </div>
              </div>
              <div class="card-body">
                <pre class="line-numbers" id="pre">{{ __("Loading Please wait")}}...</pre>
              </div>
            </div>
