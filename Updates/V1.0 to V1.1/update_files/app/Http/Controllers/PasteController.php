...
class PasteController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $content_size = number_format(strlen($request->content)/1000,2);
        if($content_size > config('settings.max_content_size_kb')){
            return redirect()->back()->withErrors('Max allowed content size is '.$content_size.'kb.')->withInput();
        }
        $paste         = new Paste();
    ...
    public function update($slug, Request $request)
    {
        ...
        $content_size = number_format(strlen($request->content)/1000,2);
        if($content_size > config('settings.max_content_size_kb')){
            return redirect()->back()->withErrors(__('Max allowed content size is ').$content_size.'kb.')->withInput();
        }
