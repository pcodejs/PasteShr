...
class PasteController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if (\Auth::check()) {
            ...
            $pastes = Paste::where(function ($q) use ($search_term) {
                ...
                $query->where('expire_time', '>', \Carbon\Carbon::now())->orWhereNull('expire_time');
            })->where('status', 1)->orderBy('created_at', 'desc')->limit(config('settings.pastes_per_page'))->get(['title', 'syntax', 'slug', 'created_at', 'password', 'expire_time', 'views']);
        }
    ...
    {
        Paste::where('slug', $slug)->where('user_id', \Auth::user()->id)->delete();
    ...
    public function report(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'reason' => 'required|clean_string|min:10|max:1000',
    ...
    {
        $trending_today = Paste::where('status', 1)->whereDate('created_at', date('Y-m-d'))->orderby('views', 'DESC')->limit(config('settings.trending_pastes_limit'))->get(['title', 'syntax', 'slug', 'created_at', 'password', 'expire_time', 'views']);

        $trending_week = Paste::where('status', 1)->whereBetween('created_at', [
        ...
        $trending_month = Paste::where('status', 1)->whereBetween('created_at', [
            \Carbon\Carbon::now()->startOfMonth(),
            \Carbon\Carbon::now()->endOfMonth(),
        ])->orderby('views', 'DESC')->limit(config('settings.trending_pastes_limit'))->get(['title', 'syntax', 'slug', 'created_at', 'password', 'expire_time', 'views']);

        $trending_year = Paste::where('status', 1)->whereBetween('created_at', [
