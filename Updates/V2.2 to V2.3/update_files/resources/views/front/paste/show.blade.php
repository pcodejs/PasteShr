        ...
        <div class="card" id="printarea">
          <div class="card-body"> @include('front.includes.messages')
            <div class="media"> @if(isset($paste->user)) <img class="mr-3 mb-3 rounded-circle img-fluid" src="{{$paste->user->avatar}}" alt="avatar" style="height: 60px"> @else <img class="mr-3 mb-3 rounded-circle img-fluid" src="{{url('img/default-avatar.png')}}" alt="avatar" style="height: 60px"> @endif
              ...
              @if(Auth::check())
              @if($paste->user_id == Auth::user()->id) <a href="{{route('paste.edit',[$paste->slug])}}" class="badge badge-info mr-2"><i class="fa fa-edit"></i> {{ __('Edit')}}</a> <a href="{{route('paste.delete',[$paste->slug])}}" class="badge badge-danger"><i class="fa fa-trash"></i> {{ __('Delete')}}</a> @else
              @if(Auth::user()->role == 1) <a href="{{route('pastes.edit',[$paste->id])}}" class="badge badge-info mr-2"><i class="fa fa-edit"></i> {{ __('Edit')}}</a> <a href="{{route('pastes.delete',[$paste->id])}}" class="badge badge-danger"><i class="fa fa-trash"></i> {{ __('Delete')}}</a> @endif
              @endif  
              @endif </div>
            ...
            @endif
            <div class="card">
              <div class="card-header"> <span class="badge badge-light">@if(!empty($paste->language)){{$paste->language->name}}@else{{strtoupper($paste->syntax)}}@endif</span> <small class="text-muted">{{$paste->content_size}} {{ __('KB')}}</small>
                <div class="pull-right"> 
                  ...
                  @if(config('settings.feature_raw') == 1 && empty($paste->password))<a href="{{route('raw',[$paste->slug])}}" class="badge badge-default">{{ __('raw')}}</a> @endif 
                  @if(config('settings.feature_download') == 1 && empty($paste->password))<a href="{{route('download',[$paste->slug])}}" class="badge badge-primary">{{ __('download')}}</a> @endif
                  @if(config('settings.feature_clone') == 1 && empty($paste->password))<a href="{{route('clone',[$paste->slug])}}" class="badge badge-secondary">{{ __('clone')}}</a> @endif
