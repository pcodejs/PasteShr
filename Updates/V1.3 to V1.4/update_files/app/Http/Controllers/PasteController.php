    ...
    public function search(Request $request)
    {
        ...
        if(\Auth::check())
        {
            $pastes = Paste::where(function($q) use ($search_term){
                ...
                $query->where('expire_time','>',\Carbon\Carbon::now())->orWhereNull('expire_time');
            })->where('status', 1)->orderBy('created_at', 'desc')->paginate(config('settings.pastes_per_page'), ['title', 'syntax', 'slug', 'created_at','password','expire_time']);
        }
        else{
            $pastes = Paste::where(function($q) use ($search_term){
                $q->orWhere('title','like','%'.$search_term.'%');
                ...
                $query->where('expire_time','>',\Carbon\Carbon::now())->orWhereNull('expire_time');
            })->where('status', 1)->orderBy('created_at', 'desc')->limit(config('settings.pastes_per_page'))->get(['title', 'syntax', 'slug', 'created_at','password','expire_time']);
        }
