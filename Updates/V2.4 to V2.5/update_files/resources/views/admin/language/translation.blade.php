          ...
          <div class="card-body">
            <form method="post">
              @csrf
                ...
                @foreach($default_words as $key => $dw)
                <div class="row">
                  <div class="form-group col-md-6">
                    <textarea class="form-control" rows="2" disabled>{{$dw}}</textarea>
                  </div>
                  <div class="form-group col-md-6">
                    <textarea class="form-control" placeholder="{{$language->name}} Translation" name="{{$key}}">@if(isset($words->$dw)){{$words->$dw}}@else{{$dw}}@endif</textarea>
                  </div>
                </div>
                @endforeach
