		...
		<div class="container">
			...
			<div class="row " data-wow-delay="0.2s">
				<div class="@if(config('settings.site_layout') == 1) col-md-9 @else col-md-12 @endif">
						...
						<ul class="list-group list-group-flush">
							@forelse($pastes as $paste)
								<li class="list-group-item">
									...
									<p><small class="text-muted">@if(isset($paste->language))
												<a href="{{$paste->language->url}}">{{$paste->language->name}}</a> @else{{$paste->syntax}}@endif |
					...
					<div class="row">
						@if(Auth::check())
							<div class=" mx-auto mt-3"> {{$pastes->appends(['keyword'=>app('request')->get('keyword')])->links()}} </div>
						@else
