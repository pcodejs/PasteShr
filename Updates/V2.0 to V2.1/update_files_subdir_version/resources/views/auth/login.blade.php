...
<main> 
  ...
  <div class="container"> 
    ...
    @if (config('settings.social_login_facebook') == 1)
        <a href="{{ route('social.login', ['facebook', 'action' => 'login']) }}" class="btn btn-primary btn-block mb-1">
             <i class="fa fa-facebook"></i> {{ __('Login with Facebook') }}
        </a>
    @endif

    @if (config('settings.social_login_twitter') == 1)
        <a href="{{ route('social.login', ['twitter', 'action' => 'login']) }}" class="btn btn-cyan btn-block mb-1">
            <i class="fa fa-twitter"></i> {{ __('Login with Twitter') }}
        </a>
    @endif

    @if (config('settings.social_login_google') == 1)
        <a href="{{ route('social.login', ['google', 'action' => 'login']) }}" class="btn btn-danger btn-block mb-1">
             <i class="fa fa-google-plus"></i> {{ __('Login with Google') }}
        </a>
    @endif
